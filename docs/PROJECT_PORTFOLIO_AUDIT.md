...
| --- | --- | --- | --- |
| 博客 | 可展示，项目事实已与源仓库对齐 | 本地改版与 Playwright 回归存在；本轮同步了 BEID / 13 tab / 1013 passed + 12 skipped / 33 契约数字 | GitHub Pages 线上是否已部署本轮改动 |
| BEID `clock` | 核心工作台可演示；本轮修了四类产品缺陷和播放器卸载计时器泄漏 | 763 tests、32 个 Node 原生边界测试、typecheck/build 通过 | B 站扫码、真机播放、Windows 安装 |
| Hardware Butler | 非硬件路径完整 | 1013 passed、12 skipped；ruff/mypy/插件校验通过 | 真实板卡、探针、编译器和供电 |
