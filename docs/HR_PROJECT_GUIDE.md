...
> 我做的是本地优先学习台，不是又一个待办清单。个人数据默认只落本机；B 站搜索、DASH 播放和学习笔记共用同一份状态。最近做了一轮系统性对抗审查，修掉专注联动被打断、续播偏好绕过、跨分P笔记跳转失效、片尾死循环等真实缺陷。回归是 768 Vitest + 32 Node 边界测试。
...
- **播放器测试大量 mock DashPlayer/playurl**，768 绿不等于 MSE 真机可用
- Tauri 打包下 MSE 播放不可用（无本地媒体代理），代码里已快速失败并提示用 Electron/Web 版  
- MSE 路径没有 playurl 刷新：媒体 URL 过期后的 403 仍会报废管线（native 路径有刷新，MSE 无等价物）——已知边界

## 面试最危险 5 问（准备好）

1. 多标签页冲突怎么 merge？→ 诚实：当前是覆盖，不是 CRDT/字段合并。  
2. 播放器里的专注为何今日页看不到？→ 两套专注状态机未合并，已知债。  
3. 768 测试等于播放稳定吗？→ 不等于；Player 大量 mock。
4. 关掉「从上次位置继续」真的不跳吗？→ 曾绕过，2026-09-11 已修复并补两条回归测试（历史回退等待偏好加载 + 应用层二次把关）。  
5. License 是什么？→ package.json MIT + 根目录 LICENSE；README 已对齐。

## 简历可用句

设计并实现本地优先学习工作台 BEID，统一任务/习惯/考研/B 站学习状态；自研 sidx 定位的 DASH/MSE 播放管线；系统化审查修复专注联动打断、续播偏好绕过、跨分P笔记跳转、片尾死循环等真实缺陷，并补回归测试（768+32）。
